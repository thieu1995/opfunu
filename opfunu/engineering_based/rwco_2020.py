...
# Source: A Test-suite of Non-Convex Constrained Optimization Problems from the Real-World and Some Baseline Results

import numpy as np
from scipy.optimize import fminbound
from opfunu.utils.constants import rwco_2020 as constant


# Industrial Chemical Processes
def heat_exchanger_network_design_1(x):
    ...
    hx[6] = x[3] * np.log(np.abs(x[7] - 100) + 1e-8) - x[3] * np.log((600 - x[6]) + 1e-8) - x[7] + x[6] + 500
    hx[7] = x[5] * np.log(np.abs(x[8] - x[6]) + 1e-8) - x[5] * np.log(600) - x[8] + x[6] + 600
    return fx, gx, hx


def heat_exchanger_network_design_2(x):
...
def blending_pooling_separation_problem(x):
...
def propane_isobutane_nbutane_nonsharp_separation(x):
    #  Propane, Isobutane, n-Butane Nonsharp Separation
    out = constant.benchmark_function(7)
    D, g, h, xmin, xmax = out["D"], out["g"], out["h"], out["xmin"], out["xmax"]
    c = np.array([[0.23947, 0.75835], [-0.0139904, -0.0661588], [0.0093514, 0.0338147],
...
def optimal_operation_alkylation_unit(x):
...
def process_synthesis_problem_2(x):
...
def process_synthesis_design_problem(x):
...
def process_flow_sheeting_problem(x):
...
def process_synthesis_problem_7(x):
    #    Process synthesis problem
    ...
    y3 = np.round(x[5])
    y4 = np.round(x[6])
    fx = (y1 - 1) ** 2 + (y2 - 1) ** 2 + (y3 - 1) ** 2 - np.log(y4 + 1) + (x1 - 1) ** 22 + (x2 - 2) ** 2 + (x3 - 3) ** 2
    gx = np.zeros(g)
...
def multi_product_batch_plant(x):
...
def weight_minimization_speed_reducer(x):
    ...
    gx[4] = 10 * x[5] ** (-3) * np.sqrt(16.91 * 10 ** 6 + (745 * x[3] / (x[1] * x[2])) ** 2) - 1100
    gx[5] = 10 * x[6] ** (-3) * np.sqrt(157.5 * 10 ** 6 + (745 * x[4] / (x[1] * x[2])) ** 2) - 850
    gx[6] = x[1] * x[2] - 40
    gx[7] = -x[0] / x[1] + 5
    gx[8] = x[0] / x[1] - 12
...
def industrial_refrigeration_system(x):
    ## Optimal Design of Industrial Refrigeration System
    out = constant.benchmark_function(16)
    D, g, h, xmin, xmax = out["D"], out["g"], out["h"], out["xmin"], out["xmax"]
    hx = 0
    gx = np.zeros(g)
    fx = 63098.88 * x[1] * x[3] * x[11] + 5441.5 * x[1] ** 2 * x[11] + 115055.5 * x[1] ** 1.664 * x[5] + 6172.27 * x[1] ** 2 * x[5] + \
...
def compression_spring_design(x):
...
def pressure_vessel_design(x):
    ...
    gx[2] = -np.pi * x[2] ** 2 * x[3] - 4 / 3 * np.pi * x[2] ** 3 + 1296000
    gx[3] = x[3] - 240
    return fx, gx, hx
...
def three_bar_truss_design_problem(x):
    out = constant.benchmark_function(20)
    D, g, h, xmin, xmax = out["D"], out["g"], out["h"], out["xmin"], out["xmax"]
    ## Three-bar truss design problem
    fx = (2 * np.sqrt(2) * x[0] + x[1]) * 100
    gx = np.zeros(g)
    gx[0] = (np.sqrt(2) * x[0] + x[1]) / (np.sqrt(2) * x[0] ** 2 + 2 * x[0] * x[1]) * 2 - 2
    gx[1] = x[1] / (np.sqrt(2) * x[0] ** 2 + 2 * x[0] * x[1]) * 2 - 2
    gx[2] = 1 / (np.sqrt(2) * x[1] + x[0]) * 2 - 2
    hx = 0
    return fx, gx, hx


def multiple_disk_clutch_brake_design_problem(x):
    ...
    hx = 0
    gx = np.zeros(g)
    fx = np.pi * (x[1] ** 2 - x[0] ** 2) * x[2] * (x[4] + 1) * rho
    gx[0] = -x[1] + x[0] + delR
...
def planetary_gear_train_design_problem(x):
    ...
    gx[4] = -((N1 + N2) * np.sin(np.pi / p) - N2 - 2 - dlt22)
    gx[5] = -((N6 - N3) * np.sin(np.pi / p) - N3 - 2 - dlt33)
    gx[6] = -((N4 + N5) * np.sin(np.pi / p) - N5 - 2 - dlt55)
    if beta == beta.real:
        gx[7] = (N3 + N5 + 2 + dlt35) ** 2 - ((N6 - N3) ** 2 + (N4 + N5) ** 2 - 2 * (N6 - N3) * (N4 + N5) * np.cos(2 * np.pi / p - beta))
    else:
        gx[7] = 1e6
...
def step_cone_pulley_problem(x):
    ...
    t = 8 * 1e-3
    ## objective function
    fx = rho * w * np.pi / 4 * (d1 ** 2 * (1 + (N1 / N) ** 2) + d2 ** 2 * (1 + (N2 / N) ** 2) + d3 ** 2 * (1 + (N3 / N) ** 2) + d4 ** 2 * (1 + (N4 / N) ** 2))
    ## constraint
...
p1 = HEND1 = heat_exchanger_network_design_1
p2 = HEND2 = heat_exchanger_network_design_2
p3 = HPP = haverly_pooling_problem
p4 = BPSP = blending_pooling_separation_problem
p5 = PINNS = propane_isobutane_nbutane_nonsharp_separation
p6 = OOAU = optimal_operation_alkylation_unit
p7 = RND = reactor_network_design
p8 = PSP2 = process_synthesis_problem_2
p9 = PSDP = process_synthesis_design_problem
p10 = PFSP = process_flow_sheeting_problem
p11 = TRP = two_reactor_problem
p12 = PSP7 = process_synthesis_problem_7
p13 = PDP = process_design_problem
p14 = MPBP = multi_product_batch_plant
p15 = WMSR = weight_minimization_speed_reducer
p16 = IRS = industrial_refrigeration_system
p17 = CSD = compression_spring_design
p18 = PVD = pressure_vessel_design
p19 = WBD = welded_beam_design
p20 = TBTDP = three_bar_truss_design_problem
p21 = MDCBDP = multiple_disk_clutch_brake_design_problem
p22 = PGTDP = planetary_gear_train_design_problem
p23 = SCPP = step_cone_pulley_problem
